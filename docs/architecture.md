...
flowchart TB
    ...
    C -->|"問題がおかしい"| Dispute["問題不備を記録<br/>学習は止めない<br/>その問題は再出題しない"]
    Grade -->|"AI 採点に不服"| GradeDispute["個別採点を保留<br/>問題自体は停止しない"]
    Grade --> Next["次の問題へ"]
    Explain --> E{"理解した？"}
    E -->|"分かった"| Next
...
本アーキテクチャで定義した構成要素の「完了」「未着手」といった具体的な進捗状況は、**[初期 PoC ロードマップ](spec/initial-poc-roadmap.md) を進捗の正本**として一元管理します。
以下はアーキテクチャ上の主要な領域と、今後本設計に向けて追加・調整すべき事項の「要約」です。

| 領域 | 本設計に向けて追加・調整する主な要素 |
|---|---|
| フロントエンド | 確定版 UI・誤操作防止・学習者アイコン切替 |
| Tauri コマンド | 学習フロー・出題選定・記録の各コマンド |
| AI 接続 | 長寿命接続・ストリーミング・`model/list` 選択・失敗復旧 |
| 問題バンク | 待機列・優先度評価・選定理由の記録 |
| 学習記録 | 日次レポートの Tauri 統合・週次・公開用統計 |
| 学習状態 | Skill・問題・学習者別状態の永続化 |
| 効果測定／保護者関与 | 遅延テスト・公開用統計・`ParentSuggestion`／`ChildResponse` |
