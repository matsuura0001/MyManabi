...
QuestionSet は、画面上・教材上のまとまりを表す。Question は採点、履歴、理解度に紐づく最小回答単位とし、1画面に必ず1 Question だけを出すとは限らない。

PDF や画像教材では、1つの問題画像に複数の小問が含まれることが多い。画像を Question ごとに細かく分けると、領域指定、回答画像との対応、後続の調整が煩雑になりやすい。そのため初期設計では、問題画像・回答画像は QuestionSet に紐づけ、Question は QuestionSet 内の小問として扱う。
...
初期版の `items` は `question_id`、`label`、`order` だけを持つ。画像内の小問ごとの詳細な座標は必須にしない。必要になった場合だけ、後から `region_hint` を追加する。

QuestionSet の材料は、大きく問題側と回答側に分ける。

- `question_materials`: 問題文テキスト、問題画像、図表など、子どもへ提示する素材
- `answer_materials`: 正答テキスト、回答画像、模範解答画像、rubric など、採点・確認・振り返りに使う素材

`問題画像` と `補助画像` は初期版では分けない。問題文と画像は共存でき、画像の役割を細かく選ばせるより、QuestionSet 内での表示順を管理するほうが入力負担が小さい。

批判的に確認する点:

- QuestionSet を入れると初期実装は少し重くなるが、1枚画像に複数回答がある教材を自然に扱える
- Question を画面単位にすると実装は一時的に楽だが、採点・履歴・理解度が粗くなる
- Question ごとに画像を直接持たせすぎると、家庭内 PoC では調整作業が過剰になる

### 5.4 LearnerSkillState
...
具体的な進捗状況については、**[初期 PoC ロードマップ](initial-poc-roadmap.md) を進捗の正本**として管理する。以下はドメイン設計上の主要な要素の要約である。

| 項目 | 本設計で必要なこと |
|---|---|
| 最初の問題 | 確認済み問題バンクと待機列から即時表示する |
| モデル選択 | `model/list` から軽量モデルを初回選択し、利用不能時だけ再選択する |
| 学習状態 | Skill、問題、学習者別状態を DATA_DIR へ保存する |
| 出題理由 | LearningPattern と SelectionDecision を記録する |
| 効果測定 | 遅延テストと公開用統計を生成する |
| 保護者関与 | ParentSuggestion と ChildResponse を記録する |
...
個別問題の調整は、取り込みレビュー画面ではなく Question 一覧または Question 詳細を主導線にする。PDF OCR からの再取り込みで直すより、昇格済みの Question / QuestionSet を直接調整する場面のほうが多いためである。手打ち問題、画像から作った問題、後から回答画像を追加した問題も同じ調整画面で扱う。

初期版で調整対象にする項目:

- 問題文・タイトル
- QuestionSet の問題側素材と回答側素材
- 画像領域の再指定
- 正答・回答画像
- 問題タイプ
- 単元・Skill
- review status: 承認 / 停止 / 無効化
- 調整理由

正答、問題の意味、要求する回答方式が変わる場合は、旧 Question を `invalidated` にして集計から除外し、新しい Question ID で作り直す。表記、画像範囲、OCR 文面、解説、許容表記の修正に留まる場合は、同じ Question ID の revision として扱える。

過去回答の再採点や複雑な revision 差分表示は初期版では必須にしない。ここを早く作り込むと、問題編集機能ではなく監査・成績訂正システムになり、PoC の重心がずれる。
...
無効化済み Question は、通常の問題一覧や出題候補から隠す。保護者向けには管理用の `無効化済み` フィルタまたは問題詳細からだけ確認できるようにする。
...
集計画面には、細かな問題文を出さず `集計対象外: 3問` のような注記を表示する。過去の正答率が変わった理由を説明するためであり、子どもへ訂正通知を強制するためではない。
...
### 17.2 決定論的採点の正規化・抽出パイプライン

決定論的採点を堅牢にするため、空白の有無や括弧の揺れによる不一致を防ぐ抽出・正規化パイプラインを本アプリ内に設ける。

- **(1) テンプレートからの抽出**: 空欄フォーマット（例: `1. (   )`）が既知の場合は、内部で正規表現（例: `^1\.\s*\(\s*(.*?)\s*\)$`）を生成し、入力された `1. ( 90° )` から核心の解答部分 `90°` だけを抽出する。
- **(2) 標準プレフィックスの除去**: テンプレートがない場合でも、先頭の `1.` `(1)` `①` などの問題番号や、全体を囲う括弧を自動で剥がす。
- **(3) 空白の除去・半角化**: 抽出後の解答文字列から全角・半角の空白をすべて除去し、英数字や記号を半角に統一して比較する。
- **(4) UIとデータの分離**: 子ども向け画面では純粋な答え（`( )` の中身）のみを入力させる専用フィールドとし、データ上も正規化後の純粋な解答のみを保存する。
...
大人が AI 採点を覆した回答や、AI / ローカルルールによる取り込み補助を保護者が修正した結果は、DATA_DIR 内のローカル評価ハーネス候補へ自動追加できるようにする。実際の評価セット採用は大人が選ぶ。
...
回答分割のように、人手を減らしたいが誤りが学習品質へ直結する処理も、ハーネスで育てる対象にする。

```yaml
answer_split_evaluation_case:
...
- 問題数と回答数の不一致率
- 誤割り当ての種類
- 教材形式、ImportProfile、ルール/モデル別の成功率

ローカルルールと AI は競合させず、単純な番号付き回答はローカルで処理し、曖昧なケースだけ AI へ渡す。AI の提案は確定情報ではなく、保護者の最終修正を正解データとして扱う。

実教材、実回答、手書き画像は著作権や個人情報を含み得るため、ローカル評価ハーネスから外部共有・公開リポジトリへ出さない。公開評価ハーネスには、合成問題、権利上公開可能と確認した問題、合成回答だけを使う。

この評価ハーネスは、初期 PoC ではまず記録形式を軽く用意し、評価や自動改善は後から強める。最初から汎用パーサーや自動承認を目指さず、保護者の修正ログを集めて、人手が多い箇所を客観的に見つける。
